...
def sphere_with_quadrilateral_faces(full_path_output_file: str, radius: float, center: tuple):
    ...
    for theta in range(theta_steps):
        for phi in range(phi_steps):
            ...
            faces.append([i + phi_steps + 1, i + phi_steps + 2, i + 1, i])
...
if __name__ == "__main__":
    sphere_with_quadrilateral_faces(
        full_path_output_file="sphere-rectangles-nocolor.off", radius=5, center=(2, 3, 5)
    )
    # --> Produces an OFF mesh of a sphere of radius 5 centered at (2,3,5)
    sphere_with_quadrilateral_faces(
        full_path_output_file="sphere-rectangles-nocolor.ply", radius=5, center=(2, 3, 5)
    )
