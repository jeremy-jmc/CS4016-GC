...
def generate_random_points(n: int, x_range: tuple, y_range: tuple, z_range: tuple) -> np.ndarray:
...
def divide_cube(cube_box: tuple) -> list:
    ...
    new_cubes = [
        (min_point, mid_point),
        (np.array([mid_point[0], min_point[1], min_point[2]]), np.array([max_point[0], mid_point[1], mid_point[2]])),
        (np.array([min_point[0], mid_point[1], min_point[2]]), np.array([mid_point[0], max_point[1], mid_point[2]])),
        (np.array([mid_point[0], mid_point[1], min_point[2]]), np.array([max_point[0], max_point[1], mid_point[2]])),
        (np.array([min_point[0], min_point[1], mid_point[2]]), np.array([mid_point[0], mid_point[1], max_point[2]])),
        (np.array([mid_point[0], min_point[1], mid_point[2]]), np.array([max_point[0], mid_point[1], max_point[2]])),
        (np.array([min_point[0], mid_point[1], mid_point[2]]), np.array([mid_point[0], max_point[1], max_point[2]])),
        (mid_point, max_point)
    ]
    return new_cubes
...
def marching_cubes(f, cube_box: tuple, precision: float = 0.025, depth: int = 0):
    if depth == 0:
        global GLOBAL_MESH
        GLOBAL_MESH = Mesh()
    ...
    p_sampling = generate_random_points(N_SAMPLING, (x_min, x_max), (y_min, y_max), (z_min, z_max))
    ...
    if x_max - x_min < precision and y_max - y_min < precision and z_max - z_min < precision:
        add_surface(f, cube_box)
        return
    
    for each_cube in divide_cube(cube_box):
        marching_cubes(f, each_cube, precision, depth + 1)
...
marching_cubes(f_json, (np.array([-10, -10, -10]), np.array([10, 10, 10])), 0.05)
